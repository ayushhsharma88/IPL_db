...
import subprocess
from pyspark.sql.functions import trim,col,sum,when,substring,lit, current_date, to_date
...
try:
    ...
    deliveries=spark.read.csv('hdfs://localhost:9000/files/deliveries/deliveries_*.csv',sep=',',header=True,inferSchema=True)
    ...
    deliveries = deliveries.withColumn("load_date", to_date(current_date()))
    ...
    # If data written successfully
    data_loaded = "Yes"

    #Write to HDFS Partition
    deliveries.write.mode("overwrite").option("header", True).csv(f"hdfs://localhost:9000{hdfs_partition_path}")
...
subprocess.run(
    "hdfs dfs -mv /files/deliveries/deliveries_*.csv /archives/deliveries",
    shell=True
)
