...
import subprocess
from pyspark.sql.functions import when,trim,col,substring,lit, current_date, to_date
...
try:
    ...
    #read file from hdfs
    matches=spark.read.csv('hdfs://localhost:9000/files/matches/matches_*.csv',sep=',',header=True,inferSchema=True)
    ...
    matches=matches.withColumn("batsmen_id",col("batsmen_id").try_cast("int"))
    matches=matches.withColumn("bowlers_id",col("bowlers_id").try_cast("int"))
    ...
    matches = matches.withColumn("load_date", to_date(current_date()))
    ...
    matches.write.mode("overwrite").option("header", True).csv(f"hdfs://localhost:9000{hdfs_partition_path}")
    ...
    try:
        conn = psycopg2.connect(
            dbname="IPL_db", user="postgres", password="password", host="localhost"
        )
        cur = conn.cursor()
...
subprocess.run(
    "hdfs dfs -mv /files/matches/matches_*.csv /archives/matches",
    shell=True
)
